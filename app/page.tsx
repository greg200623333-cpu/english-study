...
export default function Home() {
  return (
    <main className="relative min-h-screen overflow-hidden" style={{ background: '#06080d' }}>
      ...
      <nav className="glass fixed left-0 right-0 top-0 z-50 border-b border-white/8">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-6 md:py-4">
          <Link href="/" className="flex items-center gap-2 md:gap-3">
            <div className="btn-glow flex h-9 w-9 items-center justify-center rounded-xl md:h-10 md:w-10">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <HomeBriefingTrigger />
            <Link href="/login" className="hidden text-sm text-slate-400 transition hover:text-white sm:block">登录</Link>
            <Link href="/register" className="rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-3 py-1.5 text-xs font-semibold text-cyan-100 sm:px-5 sm:py-2 sm:text-sm">注册</Link>
          </div>
        </div>
      </nav>

      <section className="relative px-4 pb-16 pt-28 md:px-6 md:pb-20 md:pt-32">
        <div className="mx-auto max-w-7xl">
          <div className="grid items-center gap-8 xl:grid-cols-[1.1fr_0.9fr] xl:gap-10">
            <div>
              ...
              <h1 className="mt-6 max-w-5xl text-4xl font-black leading-tight text-slate-50 md:mt-8 md:text-5xl lg:text-7xl">
                把英语备考
                <br />
                改造成一场 <span className="gradient-text">国家经营战争模拟</span>
              </h1>
              <p className="mt-6 max-w-3xl text-base leading-8 text-slate-300 md:mt-8 md:text-lg">
                进入学习区后，默认看到的不再是普通刷题页面，而是最高指挥部、作战部署台、词汇财政部、政策输出部与法案系统。
                你的每次训练都会回流到 GDP、基建、赤字、事件流与战略曲线。
              </p>
              <div className="mt-8 flex flex-wrap gap-3 md:mt-10 md:gap-4">
                <HomeOnboardingFlow />
                <Link href="/portal" className="glass rounded-2xl px-6 py-3 text-base font-bold text-slate-100 md:px-8 md:py-4 md:text-lg" style={{ border: '1px solid rgba(255,255,255,0.1)' }}>专业模式</Link>
              </div>
              <div className="mt-4 text-sm text-slate-500">提示：个人战略方向现在支持按备考节奏随时调整。</div>
            </div>

            <div className="glass-strong rounded-[2rem] border border-white/10 p-5 md:p-6">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-xs uppercase tracking-[0.3em] text-slate-500">Command Preview</div>
                  <div className="mt-2 text-xl font-black text-slate-50 md:text-2xl">首页战略面板</div>
                </div>
              ...
              <div className="mt-5 grid gap-3 grid-cols-2">
                {strategicSignals.map((signal) => (
                  <div key={signal.label} className="rounded-[1.5rem] border border-white/8 bg-white/5 p-4">
                    <div className="text-sm font-semibold" style={{ color: signal.color }}>{signal.label}</div>
                    <div className="mt-2 text-sm leading-6 text-slate-400">{signal.desc}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="relative px-4 pb-20 md:px-6 md:pb-24">
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 text-center md:mb-12">
            <p className="text-xs uppercase tracking-[0.35em] text-cyan-300/80">Flow</p>
            <h2 className="mt-4 text-2xl font-black text-slate-50 md:text-4xl">四步进入大战略备考闭环</h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 md:gap-6 xl:grid-cols-4">
            {strategicFlow.map((item) => (
              <div key={item.step} className="glass rounded-[1.75rem] border border-white/10 p-5 md:p-6">
                ...
                <h3 className="mt-4 text-lg font-bold text-slate-50 md:mt-5 md:text-xl">{item.title}</h3>
      ...
      <section className="relative px-4 pb-20 md:px-6 md:pb-24">
        <div className="mx-auto max-w-5xl rounded-[2rem] border border-cyan-400/20 bg-[linear-gradient(135deg,rgba(34,211,238,0.12),rgba(59,130,246,0.08))] p-8 text-center backdrop-blur-md md:p-12">
          <p className="text-xs uppercase tracking-[0.35em] text-cyan-300/80">Begin Command</p>
          <h2 className="mt-4 text-2xl font-black text-slate-50 md:text-4xl">签署你的第一份动员令</h2>
          <p className="mx-auto mt-4 max-w-3xl text-base leading-8 text-slate-300 md:text-lg">学习区首页、部署台、词汇部、作文部和个人档案都已统一进入战略叙事。下一步就是正式开局。</p>
          <div className="mt-6 flex flex-wrap justify-center gap-3 md:mt-8 md:gap-4">
            <Link href="/register" className="btn-glow rounded-2xl px-6 py-3 text-base font-bold text-white md:px-8 md:py-4 md:text-lg">立即注册</Link>
            <Link href="/dashboard" className="glass rounded-2xl px-6 py-3 text-base font-bold text-slate-100 md:px-8 md:py-4 md:text-lg" style={{ border: '1px solid rgba(255,255,255,0.1)' }}>直接进入指挥部</Link>
          </div>
        </div>
      </section>
