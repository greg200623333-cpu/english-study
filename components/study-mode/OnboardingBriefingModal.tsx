      ...
      >
        ...
        <div className="shrink-0 flex items-center justify-between border-b border-white/10 px-5 py-4 md:px-8 md:py-5">
          <div>
            ...
            <h2 className="mt-1 text-xl font-black text-slate-50 md:mt-2 md:text-3xl">大战略备考版</h2>
          </div>
          <div className="text-right text-xs text-slate-400">
            <div>STEP {activeStep.step}</div>
            <div>{stepIndex + 1} / {briefingSteps.length}</div>
          </div>
        </div>
            ...
            <div className="flex flex-col justify-between gap-6">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="space-y-4"
                >
                  <div className="inline-flex items-center rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-xs text-cyan-200">
                    最高优先级战略简报
                  </div>
              ...
              <div className="flex items-center gap-3">
                {briefingSteps.map((item, index) => (
                  <div
                    ...
                    style={{ background: index <= stepIndex ? 'linear-gradient(90deg, rgba(34,211,238,1), rgba(59,130,246,1))' : 'rgba(255,255,255,0.08)' }}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="glass rounded-[1.75rem] border border-white/10 p-4 md:p-6">
              <div className="grid grid-cols-2 gap-3 text-sm text-slate-300 lg:grid-cols-1">
                <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-3 md:p-4">
                  <div className="text-xs uppercase tracking-[0.25em] text-emerald-300">GDP</div>
                  <div className="mt-1 font-bold text-slate-50 md:mt-2 md:text-xl">词汇就是经济底盘</div>
                </div>
                <div className="rounded-2xl border border-sky-400/20 bg-sky-400/10 p-3 md:p-4">
                  <div className="text-xs uppercase tracking-[0.25em] text-sky-300">WPM</div>
                  <div className="mt-1 font-bold text-slate-50 md:mt-2 md:text-xl">阅读速度决定基建效率</div>
                </div>
                <div className="rounded-2xl border border-rose-400/20 bg-rose-400/10 p-3 md:p-4">
                  <div className="text-xs uppercase tracking-[0.25em] text-rose-300">Deficit</div>
                  <div className="mt-1 font-bold text-slate-50 md:mt-2 md:text-xl">积压复习就是财政赤字</div>
                </div>
                <div className="rounded-2xl border border-amber-400/20 bg-amber-400/10 p-3 md:p-4">
                  <div className="text-xs uppercase tracking-[0.25em] text-amber-300">AP</div>
                  <div className="mt-1 font-bold text-slate-50 md:mt-2 md:text-xl">行政力驱动政策加成</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="shrink-0 flex items-center justify-between border-t border-white/10 px-5 py-4 md:px-8 md:py-5">
          <button
            ...
            className="rounded-xl border border-white/10 px-4 py-2 text-sm text-slate-400 transition hover:border-white/20 hover:text-slate-200 disabled:opacity-30"
          >
            上一步
          </button>
          {stepIndex < briefingSteps.length - 1 ? (
            <button
              type="button"
              onClick={() => setStepIndex((i) => Math.min(briefingSteps.length - 1, i + 1))}
              className="btn-glow rounded-xl px-5 py-2.5 text-sm font-semibold text-white"
            >
              继续简报
            </button>
          ) : (
